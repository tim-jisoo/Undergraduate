...
void sfs_touch(const char* path)
{
	...
	int i,j;
	int newDirBlockAllocated=0, found =0;
	...
	if(nEntry < MAX_DENTRY)
	{
		...
		{
			error_message("touch",path,-4);
			if(newDirBlockAllocated == 1) release_block(blockNo);
			return;
		}
	...
	else error_message("touch",path,-3);
}
...
void sfs_mkdir(const char* path) 
{
	...
	{	
		...
		{	
			...
			// additional new disk block will be allocated later...
			// in that case, withdraw all block allocated by using blockVector
			blockVector[vcnt++] = blockNo;
		...
		{
			error_message("mkdir",path,-4);
			if(vcnt == 1) release_block(blockVector[0]);
			return;
		}
		blockVector[vcnt++] = currDir[entryNo].sfd_ino;
		strncpy(currDir[entryNo].sfd_name,path,SFS_NAMELEN);
		...
		{
			error_message("mkdir",path,-4);
			for(i=0;i<vcnt;i++) release_block(blockVector[i]);
			return;
		}
...
void sfs_rmdir(const char* path) 
{
	...
	if(!strcmp(path,".")) error_message("rmdir",path,-8);
...
void sfs_mv(const char* src_name, const char* dst_name) 
{
	...
	else
	{
		...
		strncpy(currDir[found_entryNo].sfd_name, dst_name, SFS_NAMELEN);
...
void sfs_cpin(const char* local_path, const char* path) 
{
	...
	int found = 0, vcnt=0, directCnt=0, indirectCnt=0;
	unsigned int nEntry, directNo, entryNo, blockNo, directPtrBlockNo, dataBlockNo;
	struct sfs_inode currDirInode, newbieInode;
	struct sfs_dir currDir[SFS_DENTRYPERBLOCK];
	unsigned char buf[SFS_BLOCKSIZE];
	unsigned int directPtrBlock[128];
	unsigned int blockVector[146];  // 1:additional directory entry
					// 1:file inode
					// 15:direct pointed data block
					// 1:indirect pointed data block
					// 128:indirect -> direct pointed block
	...
	fileSize = ftell(fp);
	rewind(fp);	
	if(fileSize > MAX_FILESIZE) { error_message("cpin",path,-12); return; }
	...
	if (nEntry < MAX_DENTRY)
	{
		...
		//Update current dir Inode info. (total entry No. & direct pointer)
		//Init dir Block
		currDirInode.sfi_size += sizeof(struct sfs_dir);
		if (found == 0)
		{
		...
		//Update current directory block (insert newbie entry)
		if ((currDir[entryNo].sfd_ino = allocate_block()) == INVALID_BLOCK)
		{
			error_message("cpin", local_path, -4);
			if (vcnt == 1) release_block(blockNo);
			return;
		}
		strncpy(currDir[entryNo].sfd_name, local_path, SFS_NAMELEN);
		blockVector[vcnt++] = currDir[entryNo].sfd_ino;
	...
	else error_message("cpin", path, -3);
	
	//***STEP3: update newbie inode
	newbieInode.sfi_size = fileSize;
	...
	if(feof(fp) == 0) { error_message("cpin",path,-13); return; }
